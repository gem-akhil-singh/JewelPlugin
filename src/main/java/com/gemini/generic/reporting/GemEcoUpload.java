...
public class GemEcoUpload {
    ...
    private static void sendJewelEmailReport() {
        ...
        if (sendEmailFlag == null || (sendEmailFlag.equalsIgnoreCase("true") || sendEmailFlag.equalsIgnoreCase("y")))  {
            ...
            if (GemJarUtils.getGemJarKeyValue(GemJarConstants.JEWEL_REPORT_EMAIL_TO) != null) {
                JsonArray to = getEmailRecipient(GemJarConstants.JEWEL_REPORT_EMAIL_TO);
                if(to.size()>0) emailRequestBody.add("to", to);
            }
            if (GemJarUtils.getGemJarKeyValue(GemJarConstants.JEWEL_REPORT_EMAIL_CC) != null) {
                JsonArray cc = getEmailRecipient(GemJarConstants.JEWEL_REPORT_EMAIL_CC);
               if(cc.size() >0) emailRequestBody.add("cc", cc );
            }
            if (GemJarUtils.getGemJarKeyValue(GemJarConstants.JEWEL_REPORT_EMAIL_BCC) != null) {
                JsonArray bcc = getEmailRecipient(GemJarConstants.JEWEL_REPORT_EMAIL_BCC);
                if(bcc.size()>0 )emailRequestBody.add("bcc", bcc);
            }
            ...
            try {
                if (emailRequestBody.keySet().size() > 1) {
                    ...
                    logger.info("NO EMAIL RECEIPT ADDED");
    ...
    private static JsonArray getEmailRecipient(String recipientKey){
        ...
        return CommonUtils.convertStringInToJsonElement(recipients).isJsonArray() ?
    ...
    public static void insertSuiteData(String method, String suitePayload) {
        ...
        try {
            ...
            else if (testcase.getStatus() == 406 && testcase.getErrorMessage().contains("Project does not exist")) {
                System.out.println("It seems you don't have access to the project , Please get the access to project and try again");
                System.exit(1);
            }else {
    ...
    public static void updateJira() {
            ...
            jiraRequest.setHeader("username",GemJarUtils.getGemJarKeyValue(GemJarConstants.GEMJAR_USER_NAME));
            jiraRequest.setHeader("bridgetoken",GemJarUtils.getGemJarKeyValue(GemJarConstants.GEMJAR_REPORTING_TOKEN));
            payload.addProperty("email",GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_EMAIL));
            payload.addProperty("accessToken",GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_ACCESS_TOKEN));
            payload.addProperty("projectId",GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_PROJECT_ID));
            payload.addProperty("s_run_id",GemJarGlobalVar.s_run_id);
            String s=GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_TITLE);
            if(s!=null && !s.isBlank()){
                payload.addProperty("title",GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_TITLE));
            }
           if(GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_WORKFLOW)!= null){
                JsonArray js;
                JsonParser jsonParser=new JsonParser();
                js= (JsonArray) jsonParser.parse(GemJarUtils.getGemJarKeyValue(GemJarConstants.JIRA_WORKFLOW));
                if(!js.isEmpty()){
                    payload.add("flow",js);
                }
            }
            payload.addProperty("env", GemJarGlobalVar.environment);
            payload.addProperty("suiteName", GemJarGlobalVar.reportName);
            System.out.println("JIRA PAYLOAD =  "+ payload);
            jiraRequest.setRequestPayload(payload.toString());
            try {

...
class Suits_Details {
    ...
    private void setSuiteStatus(ArrayList<Testcase_Details> TestCase_Details) {
        ...
        if (testCaseStatSet.contains(STATUS.ERR.name())) {
            this.status = STATUS.ERR.name();
        } else if (testCaseStatSet.contains(STATUS.FAIL.name())) {
            this.status = STATUS.FAIL.name();
        }else if (testCaseStatSet.contains(STATUS.WARN.name())) {
            this.status = STATUS.WARN.name();
        }else if (testCaseStatSet.contains(STATUS.PASS.name())) {
            this.status = STATUS.PASS.name();
        }else if (testCaseStatSet.contains(STATUS.INFO.name())) {
            this.status = STATUS.INFO.name();
        } else if (testCaseStatSet.contains(STATUS.EXE.name())) {
            this.status = STATUS.EXE.name();
        } else {
            this.status = STATUS.PASS.name();
        }
    }
